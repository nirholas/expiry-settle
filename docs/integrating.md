# Integrating ExpirySettle

## From Solidity

```solidity
import {ExpirySettleHook} from "expiry-settle/src/hooks/ExpirySettleHook.sol";
...
hook.configure(key, ExpirySettleHook.Config({ /* see the README */ }));
...
const address = hookAddress("expiry-settle", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "ExpirySettle"
...
More at https://expiry-settle.pages.dev.
