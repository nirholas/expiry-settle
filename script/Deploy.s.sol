...
/**
 * @title DeployExpirySettle
 * @notice Deploys ExpirySettle deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployExpirySettle is Script {
    uint160 internal constant FLAGS = uint160(Hooks.AFTER_INITIALIZE_FLAG | Hooks.BEFORE_ADD_LIQUIDITY_FLAG | Hooks.BEFORE_SWAP_FLAG);
    ...
    function run() external {
        ...
        bytes memory creationCode = type(ExpirySettleHook).creationCode;
        ...
        console2.log("ExpirySettle ", predicted);
        ...
        vm.startBroadcast();
        ExpirySettleHook hook = new ExpirySettleHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("ExpirySettle")),
